...
- Name: TPS_addEquipment_Flow\dropdown_tests\dropdown-functionality-tests.spec.ts >> Dropdown Functionality Tests >> TC-31: Verify Status Can Be Changed from ACTIVE to OBSOLETE
- Location: tests\TPS_addEquipment_Flow\dropdown_tests\dropdown-functionality-tests.spec.ts:124:7
...
Locator: locator('select:nth-of-type(1)')
Expected: "ACTIVE"
Error: strict mode violation: locator('select:nth-of-type(1)') resolved to 3 elements:
    1) <select class="w-full px-3 py-2 bg-white text-black border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm appearance-none text-sm disabled:bg-slate-100 disabled:text-slate-500">…</select> aka getByRole('combobox').first()
    2) <select class="w-full px-3 py-2 bg-white text-black border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm appearance-none text-sm disabled:bg-slate-100 disabled:text-slate-500">…</select> aka getByRole('combobox').nth(1)
    3) <select class="w-full px-3 py-2 bg-white text-black border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm appearance-none text-sm disabled:bg-slate-100 disabled:text-slate-500">…</select> aka getByRole('combobox').nth(2)

Call log:
  - Expect "toHaveValue" with timeout 5000ms
  - waiting for locator('select:nth-of-type(1)')
...
```ts
  ...
  118 |       TestDataGenerator.generateUniqueSupplierId('TYPE1')
  119 |     );
  120 | 
  121 |     await expect(page.locator(SELECTORS.saveButton)).toBeEnabled();
  122 |   });
  123 | 
  124 |   test('TC-31: Verify Status Can Be Changed from ACTIVE to OBSOLETE', async ({ page }) => {
  125 |     const statusDropdown = page.locator(SELECTORS.statusDropdown);
  126 | 
  127 |     // Verify initial state is ACTIVE
> 128 |     await expect(statusDropdown).toHaveValue('ACTIVE');
      |                                  ^ Error: expect(locator).toHaveValue(expected) failed
  129 | 
  130 |     // Change to OBSOLETE
  131 |     await statusDropdown.selectOption('OBSOLETE');
  132 | 
  133 |     // Verify selection changed
  134 |     await expect(statusDropdown).toHaveValue('OBSOLETE');
  135 | 
  136 |     // Verify value persists
  137 |     const selectedValue = await statusDropdown.inputValue();
  138 |     expect(selectedValue).toBe('OBSOLETE');
  139 | 
  140 |     // Fill mandatory fields
  141 |     await helpers.fillMandatoryFields(
  142 |       TEST_DATA.manufacturers.existing,
  143 |       TEST_DATA.suppliers.existing,
  144 |       TestDataGenerator.generateUniqueSupplierId('STATUS1')
  145 |     );
  146 | 
  147 |     // Save equipment with OBSOLETE status
  148 |     await helpers.saveEquipment();
  149 | 
  150 |     // Verify successful save
  151 |     await page.waitForURL('**/equipment**', { timeout: TEST_CONFIG.timeouts.save });
  152 |     await expect(page.locator(SELECTORS.equipmentList)).toBeVisible();
  153 |   });
  154 | });
```
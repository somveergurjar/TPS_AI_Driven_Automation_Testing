...
- Name: TPS_addEquipment_Flow\happy_path_scenarios\happy-path-scenarios-tests.spec.ts >> Happy Path Scenarios >> TC-32: Complete Equipment Creation with All Mandatory Fields
- Location: tests\TPS_addEquipment_Flow\happy_path_scenarios\happy-path-scenarios-tests.spec.ts:12:7
...
```
Error: page.inputValue: Test timeout of 30000ms exceeded.
Call log:
  - waiting for locator('[aria-label="Auto-generated"]')
...
> 202 |     return await this.page.inputValue(SELECTORS.tpsIdField);
  ...
  230 | }
  231 | 
  232 | // Global test setup
  233 | export const globalSetup = test.extend<{
  234 |   helpers: EquipmentFormHelpers;
  235 | }>({
  236 |   helpers: async ({ page }, use) => {
  237 |     const helpers = new EquipmentFormHelpers(page);
  238 |     await use(helpers);
  239 |   },
  240 | });
  241 | 
  242 | // Test data generator for unique values
  243 | export class TestDataGenerator {
  244 |   static generateUniqueSupplierId(prefix = 'AUTO'): string {
  245 |     const timestamp = Date.now();
  246 |     return `${prefix}-${timestamp}`;
  247 |   }
  248 | 
  249 |   static generateUniqueManufacturer(prefix = 'AutoMfg'): string {
  250 |     const timestamp = Date.now();
  251 |     return `${prefix}-${timestamp}`;
  252 |   }
  253 | 
  254 |   static generateUniqueSupplier(prefix = 'AutoSupplier'): string {
  255 |     const timestamp = Date.now();
  256 |     return `${prefix}-${timestamp}`;
  257 |   }
  258 | }
```